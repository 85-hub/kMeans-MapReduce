...
def createClusters(centroids):
    for line in sys.stdin:
        ...
        # zip = iterator of tuples where the first item in each passed iterator is paired together, and then the second item in each passed iterator are paired together etc.
        for centroid, i in zip(centroids, range(3)):
            ...
            # Squared Euclidean Distance (from every point of data set to every centroid)
            cur_dist = (pow(features[0] - centroid[0], 2) + pow(features[1] - centroid[1], 2) + pow(features[2] - centroid[2],2) + pow(features[3] - centroid[3], 2))
        ...
        # Result is tab separated to circumvent ./, misunderstandings in hybrid German/English environments
        var = "%s\t%s\t%s\t%s\t%s" % (index, features[0], features[1], features[2], features[3])
        print(var)


if __name__ == "__main__":
    centroids = getCentroids('Dataset_Centroids/centroids-iter-last.csv')
